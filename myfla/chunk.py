...
class ChunkKDAFunction(torch.autograd.Function):
    ...
    def backward(
        ...
        dq, dk, dv, db, dg, dh0 = chunk_kda_bwd(
            ...
            dht=dht,
            cu_seqlens=cu_seqlens,
            initial_t=ctx.initial_t, # 新增
            T_cycle=ctx.T_cycle,   # 新增
            chunk_indices=chunk_indices,
            chunk_size=ctx.chunk_size,
        )
        ...
        return dq.to(q), dk.to(k), dv.to(v), dg.to(g), db.to(beta), dA, dbias, None, dh0, None, None, None, None, None, None, None
