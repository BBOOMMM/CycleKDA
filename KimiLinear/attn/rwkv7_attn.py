...
class RWKV7Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Training path does not use cache; use reference implementation to avoid
        # Triton autotune/kernel argument edge cases on some environments.
        if not use_cache and conv_cache is None:
            delta = token_shift_ref(hidden_states, cu_seqlens)
            conv_state = None
        else:
            need_cache = bool(use_cache)
            delta, conv_state = token_shift(
                hidden_states, cu_seqlens, output_cache=need_cache, cache=conv_cache,
            )
