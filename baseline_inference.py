...
def parse_args():
    parser = argparse.ArgumentParser(description="CycleKDA baseline inference")
...
def evaluate_and_save(args):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            preds_np = np.repeat(preds_np, args.time_stride, axis=1)
            assert preds_np.shape[1] == pred_len, f"Prediction length {preds_np.shape[1]} does not match target length {pred_len}."
